...
# Get brett Baty's batting statistics for the 2024 season
Brett_Baty <- playerid_lookup(last_name = "Baty", first_name = "Brett")

# Get Statcast data for Brett Baty 
statcast_data <- statcast_search(player_name = Brett_Baty$key_bbref, start_date = "2024-01-01", end_date = "2024-12-31")

# Filter the Statcast data for Brett Baty 
brett_baty_data <- statcast_data %>%
  filter(batter == Brett_Baty$mlbam_id)
...
hit_points <- strike_zone_plot +
  geom_point(data = brett_baty_data, aes(x = plate_x, y = plate_z, color = hit_distance_sc)) +
